...
void Server::init_player(std::vector<std::string> data_split)
{
    if (data_split.at(1) == "player") {
        ...
        for (auto& client : clients_send) {
            client.send("new player " + std::to_string(find_free_id_player()) + " " + data_split.at(2));
        }

        std::string ip = data_split.back();
        clients_send.push_back(UDPBoostNetwork::UDPSender(std::stoi(split(ip, ":").back()), split(ip, ":").front()));
        clients_send.back().send("new player " + std::to_string(find_free_id_player()) + " " + data_split.at(2) + " you");
        for (auto& entity : _ecs.getEntities()) {
            if (_ecs.hasComponent<Player>(entity)) {
        ...
        _ecs.addComponent<Player>(_ecs.getEntities().back(), Player(find_free_id_player(), data_split.at(2)));
        _ecs.addComponent<Sprite>(_ecs.getEntities().back(), Sprite("player_red", 34, 34, 5));
